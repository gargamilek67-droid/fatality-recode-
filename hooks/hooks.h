...
namespace hook {
    struct hook_t {
        ...
        hook_t() = default;
        hook_t(const uintptr_t new_func, const uintptr_t original)
            : original_(original), new_func_(new_func), index_(0), applied_(false) {
            hook_ = std::make_shared<c_detour>(original);
        }

        hook_t(const uintptr_t new_func, const uint32_t index, const uintptr_t ent)
            : new_func_(new_func), index_(index), applied_(false) {
...
#define make_hook(new_func, target_module, target) hook::hook_list.insert(std::make_pair<>((uintptr_t)(new_func), hook::hook_t((uintptr_t)(new_func), make_offset(target_module, target))))
...
#define make_hook_multi(new_func, target_module, target) hook::hook_list.insert(std::make_pair<>((uintptr_t)(new_func), hook::hook_t((uintptr_t)(new_func), make_offset(target_module, target))))
