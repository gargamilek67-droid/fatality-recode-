...
long __stdcall hook::present(IDirect3DDevice9* device, RECT* pSourceRect, RECT* pDestRect, HWND hDestWindowOverride, RGNDATA* pDirtyRegion) {
    ...
    if (!adapter) {
        ...
        draw.manage(FNV1A("child_title"), std::make_shared<font>(josefin_sans.data(), josefin_sans.size(), 11.57f, 0, 0, 0x80));
        draw.manage(FNV1A("tab_font"), std::make_shared<font>(josefin_sans.data(), josefin_sans.size(), 15.5f, 0, 0, 0x80));
