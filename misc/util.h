...
#define OFFSET_NAME(name) offsets::name
#define make_offset_simple(modulename, offset) []() -> uintptr_t { \
    static uintptr_t module_base = reinterpret_cast<uintptr_t>(GetModuleHandleA(modulename)); \
    return module_base + static_cast<uintptr_t>(offset); \
}()

#define make_offset_multi(modulename, offset) []() -> uintptr_t { \
    static volatile uintptr_t module_base = reinterpret_cast<uintptr_t>(GetModuleHandleA(modulename)); \
    return module_base + static_cast<uintptr_t>(offset); \
}()

#define make_offset(modulename, offset_name) []() -> uintptr_t { \
    static volatile uintptr_t module_base = reinterpret_cast<uintptr_t>(GetModuleHandleA(modulename)); \
    return module_base + static_cast<uintptr_t>(OFFSET_NAME(offset_name)); \
}()

#define overwrite_offset(offset_name, new_offset) []() -> uintptr_t { \
    return OFFSET_NAME(offset_name) = static_cast<uintptr_t>(new_offset); \
}()

#define get_offset(offset_name) []() -> uintptr_t { \
    return static_cast<uintptr_t>(OFFSET_NAME(offset_name)); \
}()

#define server_offset(offset_name) []() -> uintptr_t { \
    return static_cast<uintptr_t>(OFFSET_NAME(offset_name)); \
}()
...
namespace util {
	...
	uintptr_t fetch_sig(const char* targetDLL, uintptr_t targetOffset);
	uintptr_t pattern_scan(const char* moduleName, const char* pattern);
	...
	std::string format(const std::string& format, Args ... args) {
		...
		if (size_s <= 0)
		{
			*reinterpret_cast<uintptr_t*>(1337) = 1338;
		}
...
#define USER_SHARED_DATA PKUSER_SHARED_DATA(0x7FFE0000)
	...
	inline std::string base64_encode(const std::string& data) {
		static constexpr char cl[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
	...
	inline std::string base64_decode(const std::string& data) {
		...
		for (int i = 0, j = 0; i < inp_sz;) {
			const uint32_t a = data[i] == '=' ? 0 : cl[static_cast<uint32_t>(data[i++])];
			const uint32_t b = data[i] == '=' ? 0 : cl[static_cast<uint32_t>(data[i++])];
			const uint32_t c = data[i] == '=' ? 0 : cl[static_cast<uint32_t>(data[i++])];
			const uint32_t d = data[i] == '=' ? 0 : cl[static_cast<uint32_t>(data[i++])];

			const uint32_t t = (a << 18) | (b << 12) | (c << 6) | d;
